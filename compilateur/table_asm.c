	...
	for (int i = 0; i < index_tab_asm; i++) {
		if (strcmp(tab_asm[i].id, "JMP") == 0) {
			...
			if (tab_asm[i].registers[0] == -1) {
				...
				tab_asm[i].registers[0] = tab_blocs_conditionnels_get_destination_address(i + 1);
			}
		}
	}
}

void tab_asm_set_main_line(int line_asm) {
	tab_asm[0].registers[0] = line_asm;
}
...
void tab_asm_write_file() {
	...
	for (int i = 0; i < index_tab_asm; i++) {
		if (strcmp(tab_asm[i].id, "JMP") == 0 || strcmp(tab_asm[i].id, "PUSH") == 0 ||
			strcmp(tab_asm[i].id, "NEG") == 0 || strcmp(tab_asm[i].id, "POP") == 0 ||
